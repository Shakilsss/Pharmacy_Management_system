...
$sql="  SELECT customer_orders.*,  orders.customer_id,orders.medicine_name,SUM(orders.total) as total  FROM customer_orders INNER JOIN orders ON customer_orders.id= orders.customer_id where cast(customer_orders.date as Date) > (NOW() - INTERVAL 7 DAY) GROUP BY orders.customer_id";
